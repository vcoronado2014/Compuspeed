...
public class Speedometer extends View implements SpeedChangeListener {
	...
	private int VELOCIDAD_COLOR = Color.WHITE;
	private float ANCHO_VELOCIMETRO = 35f;
	private float VELOCIDAD_NORMAL = 80F;
	private static boolean USA_BARRA = true;
	private static float READING_SIZE_1 = 60f;
	...
	public Speedometer(Context context, AttributeSet attrs) {
		...
		try{
			...
			//READING_SIZE_1 = READING_SIZE;
			VELOCIDAD_COLOR = a.getColor(R.styleable.Speedometer_velocidadColor, VELOCIDAD_COLOR);
			ANCHO_VELOCIMETRO = a.getFloat(R.styleable.Speedometer_anchoVelocimetro, ANCHO_VELOCIMETRO);
		} finally{
			a.recycle();
		}
		initDrawingTools();
	...
	private void initDrawingTools(){
		...
		onMarkPaint.setStrokeWidth(ANCHO_VELOCIMETRO);
		...
		//coro
		rangeMarkPaint = new Paint();
		rangeMarkPaint.setStyle(Paint.Style.STROKE);
		rangeMarkPaint.setAntiAlias(true);
		//aplicar color de fondo rango 1
		rangeMarkPaint.setColor(Color.YELLOW);
		rangeMarkPaint.setStrokeWidth(15f);
		//rangeMarkPaint.setShadowLayer(5f, 0f, 0f, OFF_COLOR);
		//rangeMarkPaint.setShadowLayer(25, 25, 25, Color.WHITE);
		//12, 12, 12, 0xFF555555
	...
	protected void onSizeChanged(int width, int height, int oldw, int oldh) {
		...
		//originalmente era 4 el valor en vez de 3
		if (width > height){
			radius = height/3;
		}else{
			radius = width/3;
...
//		Log.d(TAG, "Height spec: " + MeasureSpec.toString(heightMeasureSpec));
	...
	public void onDraw(Canvas canvas){
		float valor = (READING_SIZE * 40) /100;

		if (USA_BARRA)
		{
			//Speedometer.READING_SIZE_1 = READING_SIZE;
			readingPaint.setTextSize(READING_SIZE);
		...
		{
			Speedometer.READING_SIZE_1 = READING_SIZE + valor;
			readingPaint.setTextSize(READING_SIZE_1);
		}
	...
	private void drawScaleRange(Canvas canvas){
		...
		double halfCircumference = radius * Math.PI;
		float anc =  -30f + ANCHO_VELOCIMETRO + 30;
		double increments = 2;
		for(int i = 4; i < this.mMaxSpeed - 4; i += increments){
			circle.addCircle(centerX, centerY, radius, Path.Direction.CW);
			canvas.drawTextOnPath("-", 
								circle, 
								(float) (i*halfCircumference/this.mMaxSpeed),
								//(float) (i*halfCircumference/(160)),
								anc-2,
								rangeMarkPaint);	
			//canvas.drawArc(oval, i, 2f, true, rangeMarkPaint);
		}
	...
	private void drawReading(Canvas canvas){
		...
		Log.v("Speedometer", message + " - " + message.length());
	}
	...
	public void _usaBarra(boolean usaBarra)
	{
		if (SpeedActivity.TAMANO_PANTALLA > 4.5)
			Speedometer.USA_BARRA = usaBarra;
